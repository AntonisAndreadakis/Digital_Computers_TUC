...
#$t0=String pointer
	...
	jal ASCII_TO_BIN		#Convert first number to integer
	jal GET_OP			#Get the operator from the string
	jal ASCII_TO_BIN		#Convert the second number to integer

SpaceIgnoreLoop:
	lb $t1, 0($t0)			#Load the first character of the string in register $t1	
	bne $t1, ' ', IgnoreSpace	#if the char is not space skip space ignoring
	addi $t0, $t0, 1		#Else increment string pointer by 1
	b SpaceIgnoreLoop		#and repeat loop
IgnoreSpace:
	lb $t1, 0($t0)			#Load the last char of the string
	bne $t1, 10, IllegalOperand	#if char!='\n' then report a bad string
	
	jal EVAL			#Do the evaluation
...
LoopStart:
	and $t5, $t1, 1			#checks when the 32th bit is 1
...
Loop1:
	lb $t1, 0($t0)			#Load the char in $t1
	bne $t1, ' ', IgnoreSpaces	#Check if char==' '
	addi $t0, $t0, 1		#and increment string pointer by 1
	b Loop1
IgnoreSpaces:
	move $20, $t1			#move the operator from $t1 to $19
	addi $t0, $t0, 1		#and increment string pointer by 1
	beq $20, '+', Operator_OK	#If the operator is '+' Op is ok
	beq $20, '-', Operator_OK	#If the operator is '-' Op is ok
	beq $20, '*', Operator_OK	#If the operator is '*' Op is ok
	beq $20, '/', Operator_OK	#If the operator is '/' Op is ok
	b IllegalOperation
